...
const $=id=>document.getElementById(id),A=window.VPP,input=$("promptInput"),findingsEl=$("findings"),compiledEl=$("compiledOutput"),sceneEl=$("sceneOutput"),roundsEl=$("rounds"),summaryEl=$("summaryLine");
...
function verdictCopy(passed,fs,triple=false){const errors=fs.filter(f=>f.severity==='error').length,warns=fs.filter(f=>f.severity==='warning').length;if(passed&&triple)return "Three passes, no blocking issues. The crew can stop arguing now. 🎬";if(passed&&warns)return `Shootable, with ${warns} small director note${warns>1?'s':''}. Nothing dramatic. 🍿`;if(passed)return "Looks shoot-ready. No preventable prompt bloopers found. 🍿";return `${errors||fs.length} blocking issue${(errors||fs.length)!==1?'s':''} found. One more take before you spend a credit. ✋`;}
function fast(opt=false){const text=input.value,scene=A.parsePrompt(text),fs=A.analyze(scene,text),sim=A.simulate(scene),passed=!fs.some(f=>f.severity==="error"),risk=Math.min(100,fs.reduce((s,f)=>s+(f.points||0),0));$("status").textContent=passed?"GREEN LIGHT 🎬":"CUT! FIX FIRST ✋";$("status").className=passed?"pass":"fail";$("risk").textContent=`Deterministic Risk: ${risk}/100`;$("confidence").textContent="Confidence: fast-mode";summaryEl.textContent=verdictCopy(passed,fs,false);metrics(sim);drawFindings(fs);sceneEl.textContent=JSON.stringify(scene,null,2);compiledEl.textContent=opt||passed?A.compile(scene):"Blocking errors found. Fix the take or run Director's Cut ×3.";roundsEl.innerHTML="<p>Quick Check finished. Press <strong>Director's Cut ×3</strong> for candidate comparison and evidence-backed review.</p>";}
function triple(){const res=A.tripleReview(input.value,$("provider").value,failures());$("status").textContent=res.passed?"GREEN LIGHT · 3× VERIFIED 🎬":"CUT! GATE BLOCKED ✋";$("status").className=res.passed?"pass":"fail";$("risk").textContent=`Review Score: ${res.score}/100`;$("confidence").textContent=`Confidence: ${res.confidence}`;summaryEl.textContent=verdictCopy(res.passed,res.findings,true);metrics(res.sim);drawFindings(res.findings);sceneEl.textContent=JSON.stringify(res.scene,null,2);compiledEl.textContent=res.finalPrompt;roundsEl.innerHTML=res.rounds.map(r=>`<div class="round"><h3>Round ${r.round} · ${r.name}</h3>${r.winner?`<p class="pass">Director pick: <strong>${r.winner}</strong></p>`:""}${(r.candidates||[]).map(c=>`<div class="candidate"><strong>${c.name}</strong> · score ${c.score} · ~${c.tok} tokens · constraints ${c.coverage}%</div>`).join("")}${(r.findings||[]).map(f=>`<div class="finding ${f.severity==='error'?'fail':f.severity==='warning'?'warn':'info'}"><strong>${f.agent} · ${f.rule_id}</strong><br>${f.message}</div>`).join("")}${r.passed!==undefined?`<p class="${r.passed?'pass':'fail'}"><strong>${r.passed?'GATE PASS · SHOOT IT':'GATE FAIL · ONE MORE TAKE'}</strong> · score ${r.score}/100 · confidence ${r.confidence}</p>`:""}</div>`).join("");}
